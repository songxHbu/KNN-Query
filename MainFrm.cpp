...
BEGIN_MESSAGE_MAP(CMainFrame, CFrameWnd)
	...
	//}}AFX_MSG_MAP
	ON_COMMAND(ID_KNN_KNN, &CMainFrame::OnKnnGTA)
END_MESSAGE_MAP()
...
CMainFrame::CMainFrame()
{
	...
	bMTA = false;  // bMTA == true, run mTA-algorithm
	bGTA = true;     // bGTA == true, run GTA-algorithm, defaule, we run GTA

	L = new  LIST[COL_NUM + 1];        // 
	...
	//////// 2015.7.30 start ////////////
	int nResponse = zlInitFunction();

	if (nResponse = iSizeofDataSetTable)
		AfxMessageBox(" Please Press Menu UniApp  \\ And then One of three MenuItems ! ");
	else
		AfxMessageBox(" zlInitFunction ()  ERROR!!!! ");
	//////// 2015.7.30 end ////////////
}
...
double CMainFrame::zlObtainSearchDistance(QUERY Query, int iQueryNum)
{
	...
	if (n==128)
	  ...
	  rgm = pow(10.0, 34); // (30 +38)/2=34
	else if(n==25)
	  rgm = pow(10.0, 31); // (24 +38)/2=31
	else
       rgm = 1;
...
2007.01.22 */				
	...
	double rd_ADM = pow(2.0*iTopN/Ztmp[iAllMeadian].f , 1/COL_NUM)*Ztmp[iAllMeadian].r;
		   //rd_ADM = pow(2*iTopN/freAMD[iAllMeadian] , 1/COL_NUM)*rAMD[iAllMeadian];



    //----------To computer search distance r
	//每次这个距离都是0，说明查询点和知识库的点很接近
	...
	else
	{
		...
		double PowDisQQp =1.0 ; //denominator; //分母dinf(Q,Qi)^^a 
		//double * w;
...
/////////
	double r[2000];    //2015.3.25
	double dis[2000];  //2015.3.25
	//double r[100+1];     //2015.3.25
	//double dis[100+1];   //2015.3.25
...
iGetTopNTupleTime = GetTickCount()- iGetTopNTupleTime;
	...
	if(bAddEP || iFrequence_Opt<m_TopN )
	{
		...
		int ilen = strlen(cDblToStr);
		r_Opt = r_Opt + EPCINO* pow(10.0, (ilen-1) );
		Q[QueryNum].r_Opt = r_Opt;
		iFrequence_Opt =  zlExeSelectCount(Q[QueryNum], r_Opt);
...
void CMainFrame::OnInsert() 
{
		...
		//********************************************
			...
			UCHAR sz_DSN[] = "DSN_QueryStream";
			UCHAR sz_UID[] = "sa";
			UCHAR sz_PWD[] = "sxzhy+dd";   //txgstxh
			   ...
			   sprintf( cttt, "source: %2.10f   buffer: '%s'   decimal: %d   sign: %d\n",
				...
				/*********
					...
					**********/
					...
					source = q[i][qnum] - rd;
					buffer = _fcvt( source, 17, &decimal, &sign );
					dL = 0.00001*pow(10.0,decimal-1);

					source = q[i][qnum] + rd;
					buffer = _fcvt( source, 17, &decimal, &sign );
					dR = 0.00001*pow(10.0,decimal-1);
...
////////// TEST START,  NO USEFUL ////////////////
		...
		char cDblToStr[50] ="";
			...
			//////////median of ALL density 
				...
				rd_ADM = pow(2.0*iTopN/freAMD[iAllMeadian] , 1/COL_NUM)*rAMD[iAllMeadian];
...
double CMainFrame::zlVolumeOfUnion(int iCountRt, int iDimension, ZLRECT *S)
{
	...
	double VFactor;
	if(n==104)
	  VFactor = 5.0 * pow(10.0, 37);
	else if(n==50)
	  VFactor = pow(10.0, 34); // (30 +38)/2=34
	else if(n==25)
	  VFactor = pow(10.0, 31); // (24 +38)/2=31
...
{
	__int64 iResult;
	unsigned __int64  iFenZi=1; //分子 //unsigned __int64 
	unsigned __int64  iFenMu =1; //分母
	int k;
...
// 2.-----------  Select all queires from workload table ---------------
	...
	//for(i=9595; i<iSizeofWorkload; i++)  //1207 //iNumUpdateKB =902; 
	{
		//if(i==1445)
			//int isahf=0;
		//if(i==2594)
	   ...
	   //top_N_time = GetTickCount();
		if (ituples_cnt > 20){//2016.0805 by song
...
int CMainFrame::zlSelectAllFromKB(char *KBTableName)
{
    ...
    */
	...
	{  //	retcode = SQLExecDirect
		...
		while (TRUE) 
		{
			...
			if (retcode == SQL_SUCCESS || retcode == SQL_SUCCESS_WITH_INFO)
			{
					...
					char cDblToStr[1024] = "";//修正r值
					sprintf(cDblToStr, "%.0f", Z[iQryIdx].r);
					int ilen = strlen(cDblToStr);//2016.0806 by song
					//Z[iQryIdx].r = (float)sr + EPCINO* pow(10.0, (ilen - 1));//2019 by song去掉修正，128维度的每一维度都是整数值，且数值不大
					Z[iQryIdx].r = (float)sr;//2019 by song
					Z[iQryIdx].f		= sf;
					Z[iQryIdx].c		= sc;
					Z[iQryIdx].d		= sd;
	...
	//zlCloseDB();
	//----- body-----------------	
	////////////////////////////////////////////////   2015.07.29 , add start ////////////////////////////////////
	// we need to obtain the top-N tuples of the profile Query S(Q, r, N) :  Z[iQryIdx].x[i] = fCol[i];	
	// there are two ways to get the top-N tuples: 1. from the tInD[ ] ; 2 select them from a Table ：//the profile Query and the tids of tuples in S(Q, r, N) 
	// iRowCnt_KBP

	bool bInRect = false;
	int j;
	iQryIdx = 0;   // int iQryIdx = 0; its based 0 
	TUPLE * tmp_tpl; //the tuples in  S(Q, r, N) 

	int k = 0;
	int frequence_Opt = 0;

	for (iQryIdx = 0; iQryIdx < iRowCnt_KBP; iQryIdx++)  // For each  the profile Query S(Q, r, N)
	{
		k = 0;
		Z[iQryIdx].pTID = new int[Z[iQryIdx].f + 1];

		tmp_tpl = new TUPLE[Z[iQryIdx].f + 1]; //the tuples in  S(Q, r, N) 

		for (i = 1; i <= iSizeofDataSetTable; i++)     // Get the top-N tuple of S(Q, r, N)
		{
			///===========
			bInRect = true;
			for (j = 0; j < COL_NUM; j++)
			{
				...
				if (tInD[i].x[j] < Z[iQryIdx].x[j] - Z[iQryIdx].r || Z[iQryIdx].x[j] + Z[iQryIdx].r < tInD[i].x[j])
				{
					bInRect = false;
					break;   //for j
				}
			...
			if (bInRect&&k<Z[iQryIdx].f)//&&k<Z[iQryIdx].f   2016.0809 by song
			{
				k++;
				tmp_tpl[k] = tInD[i];
			}
		...
		frequence_Opt = zlGetTopNTupleOfProfileQuery(iQryIdx, k);

		for (i = 0; i < Z[iQryIdx].N; i++)
		{
			Z[iQryIdx].pTID[t[i].num] = t[i].ID;   // t[i] is the t[i].num-th tuple in top-N results , its ID is  t[i].ID
		}

		delete[] tmp_tpl;

		int asdfafsda = 0;
		// ?? need to do t[] such that they are top-N results.
...
/********************
	...
	SCRi = SCR(?i),  denote  Ξ = {SCR1 , SCR2, …, SCRM}. Let   i. cn := k 
	if its profile regions is contained in SCRk . P and Ξ will be stored in main memory. 
	For each region SCRi , Retrieve all tuples in scr[i] it and save them in meory. 

*********************/

	//2015.7.30. zl add. get top-N tuples for each Profile query.
	// 
	int CMainFrame::zlGetTopNTupleOfProfileQuery(int QueryNum, int tuplesNum)
	{
		...
		//-----------2015. 08.04 -----------
		double *  r = new double[iRowCnt + 1];
		double * dis = new double[iRowCnt + 1];
		int * tIndex = new int[iRowCnt + 1];
		...
		iCandidateNum = tuplesNum;
		//In this function: iRowCnt >= iTopN;
		//calculate the distance from Z[QueryNum] to t[j], save it in t[j].d
		for (k = 0; k<iCandidateNum; k++)
		{
			for (j = 0; j<COL_NUM; j++)
				x[j] = fabs(t[k].x[j] - Z[QueryNum].x[j]);
		...
		for (j = 0; j < iRowCnt; j++)
		{
			dTemp = dis[j]; //song 2015.08.04

			for (k = j + 1; k < iRowCnt; k++)
			{
				if (dis[k] < dTemp)
				{
					dTemp = dis[k];//song 2015.08.06
					ftmp = dis[j];                    dis[j] = dis[k];                dis[k] = ftmp;//song 2015.08.04
					iNumTmp = t[j].ID;  t[j].ID = t[k].ID;   t[k].ID = iNumTmp;


					//tIndex[j]=k;		
				}
			}
			t[j].num = j;//song 2015.08.04
			tIndex[j] = t[j].num;
		}
...
//------- 1. Find out Z with minimun priority.  
	...
	temPry = (c*iFactor)/(fabs(double(w-Z[0].d)) +1);
    minPriorty = temPry;
    iminPrNum = 0;
	for(i = 1; i<iRowCnt_KBP; i++)
	{
		temPry = (c*iFactor)/(fabs(double(w-Z[i].d)) +1);
...
void CMainFrame::OnKnnGTA() //  
{

	bool bReturn = false;
	int old_m = 0;
	iSizeOfBall = 0;  //which has been  changed member varible.
	iSizeofCandidateSet = 0;
	iSizeOf_n_square = 0;
	...
	*	m_TopN = TableName.m_TopN;
		...
		//for(i=2594; i<iSizeofWorkload; i++)  //1207 //iNumUpdateKB =902; 
	{
		...
		Q[i].ante_SizeKB = iRowCnt_KBP;
		/// ---------------   total_time = GetTickCount();   // start 
		//$$$$$$$$$$$$$$$$$$$$ 2015.08.18 start song $$$$$$$$$$$$$$$$$$$$
		LARGE_INTEGER total_time_countstart;
		LARGE_INTEGER total_time_countend;
		__int64 total_time_timediff;
		LARGE_INTEGER total_time_frequent;
		QueryPerformanceCounter(&total_time_countstart);
		//$$$$$$$$$$$$$$$$$$$$2015.08.18 end song$$$$$$$$$$$$$$$$$$$$

		//----------------- 3. get search distance r for each query
		//--- call function: zlObtainSearchDistance(QUERY Query, int iQueryNum)
		//shortTicks = GetTickCount();
		//-----  r_time = GetTickCount() - r_time;  //by song 2015.08.18
		//$$$$$$$$$$$$$$$$$$$$ 2015.08.18 start song $$$$$$$$$$$$$$$$$$$$
		...
		r_time = 1000 * ((double)timediff / (double)frequent.QuadPart);  //get the performance time (ms)
		//$$$$$$$$$$$$$$$$$$$$2015.08.18 end song$$$$$$$$$$$$$$$$$$$$
		...
		int f_first = 0, f_rst = 0;
		//if (Q[i].case1 == 1 && Zprime.cn > 0) // i.e., if(Zp.dis_Q_Qp == 0) // if Q=Q'; Q[iQueryNum].case1 = 1; //case (a) of Fig.2 in Paper JIIS 2010

		if (Q[i].case1 == 1 && m_TopN <= Zprime.N) // i.e., if(Zp.dis_Q_Qp == 0) // if Q=Q'; Q[iQueryNum].case1 = 1; //case (a) of Fig.2 in Paper JIIS 2010
		{
			//Q = Q[i]; 
			// In this case, we can obtain directly the the same results as that of Q'. where Q' is the profile query in the KB, 
			// and the results of  Q' have been obtained.

			//IF the new query Q is the same as the profile query Q', i.e., (Q, N) = (Q', N) or Q.N < Q'.N
			//Then we can obtain the  Q.N results of Q directly by Zprime.pTID

			// ---- in this case , we measure the "top_N_time"  start ----
			//$$$$$$$$$$$$$$$$$$$$ 2015.08.18 start song $$$$$$$$$$$$$$$$$$$$
			LARGE_INTEGER top_N_time_countstart;
			LARGE_INTEGER top_N_time_countend;
			__int64 top_N_time_timediff;
			LARGE_INTEGER top_N_time_frequent;
			QueryPerformanceCounter(&top_N_time_countstart);
			//$$$$$$$$$$$$$$$$$$$$2015.08.18 end song$$$$$$$$$$$$$$$$$$$$

			delete[] t;
			t = new TUPLE[m_TopN + 1];
			//int iTID = 0;
			for (j = 0; j < m_TopN; j++)
			{
				//iTID = Zprime.pTID[j];             // pTID[j] is the TID of the top-j tuple
				//t[j] = tInD[iTID];                       // for tInD[j];   j == tInD[j].ID
				t[j] = tInD[Zprime.pTID[j]];      // for tInD[j];   j == tInD[j].ID
			}


			//$$$$$$$$$$$$$$$$$$$$ 2015.08.18 start song $$$$$$$$$$$$$$$$$$$$
			QueryPerformanceCounter(&top_N_time_countend);
			top_N_time_timediff = (__int64)top_N_time_countend.QuadPart - (__int64)top_N_time_countstart.QuadPart;
			QueryPerformanceFrequency(&top_N_time_frequent);
			top_N_time = 1000 * ((double)top_N_time_timediff / (double)top_N_time_frequent.QuadPart);  //get the performance time (ms)
			//$$$$$$$$$$$$$$$$$$$$2015.08.18 end song$$$$$$$$$$$$$$$$$$$$
			// ---- in this case , we measure the "top_N_time"  end ----

			// ???? ------we can use a file to display the top-N tuples to TEST whether our results are the same as those of the Naive method.
			//????  -----  or use the tool of VS 2013

		} //if (Q[i].case1 == 1 && Zprime.cn > 0)
		else
		{
			// In the cases below:  
			//(Q[i].case1 == 1 && m_TopN > Zprime.N) 
			//Q[i].case2a = 1; //// case (b) of Fig.2 in Paper JIIS 2010
			//Q[i].case2b = 1; //// case (c) of Fig.2 in Paper JIIS 2010
			//the query point Q[i] is not in the KB, get its Candidate Set by calling the following function 
			// pApp->zlGetCandidateSet( ) can get tuples in n-square S = S(Q, r) 

			// ----  we measure the "candidate_time"  start ----
			//$$$$$$$$$$$$$$$$$$$$ 2015.08.18 start song $$$$$$$$$$$$$$$$$$$$
			LARGE_INTEGER candidate_time_countstart;
			LARGE_INTEGER candidate_time_countend;
			__int64 candidate_time_timediff;
			LARGE_INTEGER candidate_time_frequent;
			QueryPerformanceCounter(&candidate_time_countstart);
			//$$$$$$$$$$$$$$$$$$$$2015.08.18 end song$$$$$$$$$$$$$$$$$$$$
			char cDblToStr[1024] = "";
			sprintf(cDblToStr, "%.0f", r);

			//int ilen = strlen(cDblToStr);
			//r = r + EPCINO* pow(10.0, (ilen - 1));//2016.0806 by song 得到候选集，见论文p20,分为case1,case2,case3三种情况
			ituples_cnt = pApp->zlGetCandidateSet(Q[i], r);  //(QUERY Query, double r) 

			
			//$$$$$$$$$$$$$$$$$$$$ 2015.08.18 start song $$$$$$$$$$$$$$$$$$$$
			QueryPerformanceCounter(&candidate_time_countend);
			candidate_time_timediff = (__int64)candidate_time_countend.QuadPart - (__int64)candidate_time_countstart.QuadPart;
			QueryPerformanceFrequency(&candidate_time_frequent);
			candidate_time = 1000 * ((double)candidate_time_timediff / (double)candidate_time_frequent.QuadPart);  //get the performance time (ms)
			//$$$$$$$$$$$$$$$$$$$$2015.08.18 end song$$$$$$$$$$$$$$$$$$$$
			// ----  we measure the "candidate_time"  end ----

			///------------ zl. 2015.08.11 start ---------------
			// zlGetTuplesInBall()  and  zlGetTopNTupleUniA() come from Liu Feifei. 
			//  zlGetTuplesInBall  return     iSizeofCandidateSet, which is the number of tuples in the Ball or Circle  that is the subset of n-squaer S = S(Q, r) 
			// In Liu Feifei:  iSizeOfBall == iSizeofCandidateSet; the number of tuples in ball , the ball is the subset of n-square

			// ----  we measure the "top_N_time"  start ----
			//$$$$$$$$$$$$$$$$$$$$ 2015.08.18 start song $$$$$$$$$$$$$$$$$$$$
			LARGE_INTEGER top_N_time_countstart;
			LARGE_INTEGER top_N_time_countend;
			__int64 top_N_time_timediff;
			...
			//$$$$$$$$$$$$$$$$$$$$2015.08.18 end song$$$$$$$$$$$$$$$$$$$$
			//根据得到候选集的情况做后续处理
			//如果得到0个候选，那说明该查询Q没有BSR且Q和所有BSR交集为空，则直接运行zlGetCandidateSetBSR()
			if (ituples_cnt <= 0)   ////   |S(Q, r)| = 0,  the n-squaer S = S(Q, r)  of  Q[i] has NOT a super set in the small contianing squares of clusters {scr_CLS[]}
			{
				if (bMTA)
					iSizeOfBall = GetTASameResult(Q[i], i);  // mTA  Liu Feifei 
				else if (bGTA)  //defult we run GTA 
					ituples_cnt = zlGetCandidateSetBSR(Q[i], i);
			}
			//如果得到的候选有，但是数量不够TopN，则运行zlGetCandidateSetBSR_Square()
			else if ((0 < ituples_cnt) && (ituples_cnt < m_TopN)) // that is the number of tuples in the  n-square S = S(Q, r) is 0 <  |S(Q, r)| < m_TopN
			{
				//call GTA or mTA
				// QUERY Query = Q[i],  int iQueryNum = i, double dSd = r,  iSizeOf_n_square = ituples_cnt
				//ituples_cnt = zlGetCandidateSetBSR_Square(QUERY Query, int iQueryNum, double dSd, int iSizeOf_n_square)
				// zlGetCandidateSetBSR_Square  can garantee  iSizeOfBall >= m_TopN 
				if (bMTA)
					iSizeOfBall = GetTASameResult_Square(Q[i], i, r, ituples_cnt);  // mTA  Liu Feifei
				else if (bGTA) //defult we run GTA 
					ituples_cnt = zlGetCandidateSetBSR_Square(Q[i], i, r, ituples_cnt);
			}
			//如果得到的候选足够TopN，则直接得到结果
			else // that is (ituples_cnt >= m_TopN) 
			{
				//r = Q[i].r;
				for (j = 0; j <COL_NUM; j++) //i means attri --- Attribute_i 
				{
					S.a[j] = Q[i].x[j] - r;
					S.b[j] = Q[i].x[j] + r;
				}
				...
				delete[] tpl;
				tpl = new TUPLE[ituples_cnt + 1];
				memset(tpl, 0, sizeof(TUPLE)*(ituples_cnt + 1));


				for (ii = 0; ii<ituples_cnt; ii++)
				{

					tpl[ii] = pApp->tpl[ii + 1];
					tpl[ii].bIN = false; //2016.0809 by song
					tpl[ii].bOK = false;//2016.0809 by song
					for (jj = 0; jj<COL_NUM; jj++)
						tpl[ii].x[jj] = pApp->tpl[ii + 1].x[jj + 1];
				...
				{
					iSizeOfBall = 0;
					iSizeOfBall = zlGetTuplesInBall(tpl[0], ituples_cnt, Q[i], i, S, r);  // zlGetTuplesInBall  return iSizeofCandidateSet; 
				}
				if (iSizeOfBall < m_TopN)
				{
					if (bMTA)
						iSizeOfBall = GetTASameResult_Square(Q[i], i, r, ituples_cnt);  // mTA  Liu Feifei
					else if (bGTA) //defult we run GTA  得到结果集不够，再扩展
						ituples_cnt = zlGetCandidateSetBSR_Square(Q[i], i, r, ituples_cnt);	// zlGetCandidateSetBSR_Square  can garantee  iSizeOfBall >= m_TopN 
						
				}

			}
			//error by song 2016.0806
			if (iSizeOfBall >= m_TopN)
			{
				///call  int CMainFrame::zlGetTopNTupleUniA(int QueryNum, int tuplesNum, QUERY Query)
				/// 5. Get top-N tuples from the candidate set :  for tpl[].bOK == true
				if (bGTA) //defult we run GTA 
					iAnswerNum = zlGetTopNTupleUniA(i, ituples_cnt, Q[i]); //	
			}
			else
			{
				sprintf(temp, " iSizeOfBall < m_TopN  Error !! iSizeOfBall = %i", iSizeOfBall);
				AfxMessageBox(temp);
			}
			...
			// ----  we measure the "top_N_time"  end ----
			///------------ zl. 2015.08.11 end ---------------
		}
		f_first = ituples_cnt;/// by song  2016.08.24
		Q[i].f = ituples_cnt;/// by song  2016.08.24
		/******************************************************************************
		...
		/// ituples_cnt = pApp->zlExeSelectQ_r_LRC(Q[i], r);
		/// we use mTA ( Q[i].r ) or GTA ( Q[i].r ) to replace  pApp->zlExeSelectQ_r_LRC(Q[i], r)
		if (ituples_cnt >= m_TopN)
		{
		f_rst = ituples_cnt;
		Q[i].f = ituples_cnt;
		...
		else if (iDimention > 24)  // deal with Lsi25D, Lsi50D, and Lsi 104D
		{
		bAddEP = true;
		char cDblToStr[1024] = "";
		sprintf(cDblToStr, "%.0f", r);
		...
		********************************************************/
		//$$$$$$$$$$$$$$$$$$$$ 2015.08.18 start song $$$$$$$$$$$$$$$$$$$$
		QueryPerformanceCounter(&total_time_countend);
		total_time_timediff = (__int64)total_time_countend.QuadPart - (__int64)total_time_countstart.QuadPart;
		QueryPerformanceFrequency(&total_time_frequent);
		total_time = 1000 * ((double)total_time_timediff / (double)total_time_frequent.QuadPart);  //get the performance time (ms)
		//$$$$$$$$$$$$$$$$$$$$2015.08.18 end song$$$$$$$$$$$$$$$$$$$$
		///---            total_time = GetTickCount() - total_time;   end  -------


		//**********************************************************************
		//------------- 5'. for unforgettable, Insert new Z into KB Profile
		//------------- 5'. for forgettable , we update the KB Profile
		///------------ , which including, Insert new Z, or raplace old one that priority is lowest by new one
	...
	//delete [] szSql;
	///----------- body end ---------------
	AfxMessageBox("OnMenuGTA OK OK OK !!!!");

}
/////this function GetTASameResult comes from Liu Feifei, which is the mTA algorithm  
int CMainFrame::GetTASameResult(QUERY Query, int iQueryNum)
{
	int n = COL_NUM;
	QUERY Qry = Query;
	//ZLRECT S; //the search n-square of Qry;  S = [a(0), b(0)]x...x[a(n-1), b(n-1)]
	bool bInRect = true;
	int iTopN = Qry.N;
	int i, j, k;
	// the index of a[ka] and b[ka] for ZLRECT S. 
	int ka[COL_NUM + 1], kb[COL_NUM + 1];
	bool kaExhaust[COL_NUM + 1], kbExhaust[COL_NUM + 1];
	//char temp[1024];
	...
	/// remember the above probe:  a[i] < = Qry.x[i] <= b[i] 
	double a2[COL_NUM + 1], a1[COL_NUM + 1], b1[COL_NUM + 1], b2[COL_NUM + 1]; //

	double r = 0;    //the radius of Ball B(Q, r, d_max) 
	double r_min = 0; // min(a1-a2, b2-b1) for each dim
	double r_maxmin = 0;    //max(r_min) 

	//double x[COL_NUM +1];

	delete[] tpl;
	tpl = new TUPLE[iSizeofDataSetTable + 1];   //candidate tuples for tpl[].bOK = true
	int itplIdx = 0;          // count for have been seen tuples, i.e., tpl[]	 
	for (i = 0; i<(iSizeofDataSetTable + 1); i++)
	{
		memset(&tpl[i], 0, sizeof(TUPLE));
	}


	//TUPLE t_tmp; 
	char cID[581012]; // be used to remove repeat tuples
	memset(cID, '\0', 581012);

	iSizeofCandidateSet = 0;
	iSortedCount = iRandomCount = 0;
	iSizeOf_n_square = 0;
	int low, upp, location;
	double UpperBound[COL_NUM + 1];
	double UpperBoundSum;
	double N;

	iSeenCount = 0;

	//-----  1. Location of QUERY Qry = (q[0],..., q[i], ..., q[COL_NUM-1])------------------------------

	for (i = 0; i <COL_NUM; i++) //i means attri --- Attribute_i 
	{
		//Li.t[j] , j is based 0 , Li.t[j] from small to large  
		if (Qry.x[i] <= L[i].t[0].a)  // q[i] <= MIN(Ai), where  q[i] is Qry.x[i] 
		{
			ka[i] = -1;  //never access it,
			kb[i] = 0;   //will  access it by kb[i]++ 
			r_min = (L[i].t[0].a - Qry.x[i]);   //MIN(Ai)-q[i]
			r_maxmin = __max(r_maxmin, r_min);     // r_maxmin initializes 0
			a2[i] = a1[i] = Qry.x[i] - 10000 * r_maxmin - 1000; // a2[i] = a1[i] = -o|o, infinite Large 
			b2[i] = b1[i] = L[i].t[0].a;                  //  b2[i] = b1[i] = MIN(Ai)==m

			//For both sorted and random accesses
			//(1) Sorted Access, (2) Random Access
			// Random Access other attr _i , and save it in buffer: tpl[] 
			if (cID[L[i].t[0].ID] == 0)
			{
				iSeenCount++;

				cID[L[i].t[0].ID] = 1; //remove repeat tuples
				tpl[itplIdx].ID = L[i].t[0].ID;
				tpl[itplIdx].x[i] = L[i].t[0].a; // Sorted Access
				//tpl[itplIdx].d =GetTupleD(tpl[itplIdx],Qry);
				tpl[itplIdx].d_max = tpl[itplIdx].d;
				iSortedCount++;
				// if(tpl[itplIdx].ID==2179)
				// AfxMessageBox("找到了"); 

				for (k = 0; k<i; k++) // other Attribute index k , Random Access
				{
					tpl[itplIdx].x[k] = tInD[tpl[itplIdx].ID].x[k];
					iRandomCount++;
				}
				for (k = i + 1; k<COL_NUM; k++) // other Attribute index k, Random Access
				{
					tpl[itplIdx].x[k] = tInD[tpl[itplIdx].ID].x[k];
					iRandomCount++;
				}
				tpl[itplIdx].d = GetTupleD(tpl[itplIdx], Qry);
				tpl[itplIdx].d_max = tpl[itplIdx].d;
				itplIdx++; //tpl's index is based 0
			}


		}
		else if (Qry.x[i] >= L[i].t[iSizeofDataSetTable - 1].a) // q[i] >=  MAX(Ai)
		{
			ka[i] = iSizeofDataSetTable - 1; //will use it by ka[i]--
			kb[i] = iSizeofDataSetTable;  //will not access it  
			r_min = (Qry.x[i] - L[i].t[iSizeofDataSetTable - 1].a);  // q[i] - MAX(Ai) 
			r_maxmin = __max(r_maxmin, r_min);                        //
			a2[i] = a1[i] = L[i].t[iSizeofDataSetTable - 1].a;          // a2[i] = a1[i] = MAX(Ai)
			b2[i] = b1[i] = Qry.x[i] + 10000 * r_maxmin + 1000;       // b2[i] = b1[i] = +o|o, infinite

			//For both sorted and random accesses
			//(1) Sorted Access, (2) Random Access
			// Random Access other attr _i , and save it in buffer: tpl[] 
			if (cID[L[i].t[iSizeofDataSetTable - 1].ID] == 0)
			{
				iSeenCount++;

				cID[L[i].t[iSizeofDataSetTable - 1].ID] = 1; //remove repeat tuples
				tpl[itplIdx].ID = L[i].t[iSizeofDataSetTable - 1].ID;
				tpl[itplIdx].x[i] = L[i].t[iSizeofDataSetTable - 1].a; // Sorted Access
				//tpl[itplIdx].d =GetTupleD(tpl[itplIdx],Qry);
				//tpl[itplIdx].d_max = tpl[itplIdx].d;
		...
		else  // i.e., in the middle //  MIN(Ai) < q[i] < MAX(Ai)
		{
			for (j = 0; j < iSizeofDataSetTable - 1; j++)   // since j+1 = iSizeofDataSetTable-1
			{
				//Li.t[j] , j is based 0; find out a[i][j]<=q[i] <=a[i][j+1]
				if ((L[i].t[j].a <= Qry.x[i]) && (Qry.x[i] <= L[i].t[j + 1].a))
				{
					ka[i] = j;      //save the location by ka[i], kb[i]
					kb[i] = j + 1;
					r_min = __min((Qry.x[i] - L[i].t[j].a), (L[i].t[j + 1].a - Qry.x[i])); //q[i]-a[i][j], a[i][j+1]-q[i] 
					r_maxmin = __max(r_maxmin, r_min);

					a2[i] = a1[i] = L[i].t[j].a;  // save values that will be used to create two intervals 
					b2[i] = b1[i] = L[i].t[j + 1].a;

					//----------- stored the seen tuples
					//For both sorted and random accesses
					//(1) Sorted Access, (2) Random Access 
					// for j 
					if (cID[L[i].t[j].ID] == 0)
					{
						...
						cID[L[i].t[j].ID] = 1; //remove repeat tuples
						tpl[itplIdx].ID = L[i].t[j].ID;
						tpl[itplIdx].x[i] = L[i].t[j].a; // Sorted Access
						// tpl[itplIdx].d =GetTupleD(tpl[itplIdx],Qry);
						//tpl[itplIdx].d_max = tpl[itplIdx].d;
						iSortedCount++;
						for (k = 0; k<i; k++) // other Attribute index k , Random Access
					...
					//(1) Sorted Access, (2) Random Access 
					// for j+1 
					if (cID[L[i].t[j + 1].ID] == 0)
					{
						cID[L[i].t[j + 1].ID] = 1; //remove repeat tuples
						tpl[itplIdx].ID = L[i].t[j + 1].ID;
						tpl[itplIdx].x[i] = L[i].t[j + 1].a; // Sorted Access
						//tpl[itplIdx].d =GetTupleD(tpl[itplIdx],Qry);
						//tpl[itplIdx].d_max = tpl[itplIdx].d;
						iSortedCount++;
	...
	////////////////////--------- construct the first n-square S , and two intervals 
	TUPLE TmpTuple;
	for (j = 0; j<itplIdx - 1; j++)
	{
		//	if(tpl[j].ID==15982)
		//	AfxMessageBox("找到了");
		/*  if(tpl[j].ID==2179)
		AfxMessageBox("找到了"); */
		for (k = j + 1; k<itplIdx; k++)
		{
			if (tpl[k].d < tpl[j].d)
			{
				TmpTuple = tpl[j];     tpl[j] = tpl[k];      tpl[k] = TmpTuple;
				//iNumTmp  = t[j].num; t[j].num = t[k].num;  t[k].num =iNumTmp; 
			}
		}
	}
	// if(tpl[itplIdx-1].ID==15982)
	//	AfxMessageBox("找到了");
	/*if(tpl[itplIdx-1].ID==2179)
	AfxMessageBox("找到了"); */



	//note: two intervals should be the same as the n-square S 

	//////////////////////////// Get Candidate tuples from the first seen tuples 

	//	int  iSizeOfBall = 0 ;

	//iSizeOfBall = zlGetTuplesInBall(tpl[0], itplIdx,  Qry, iQueryNum, S, r); 

	/*	if (iSizeofCandidateSet >= iTopN)
	{
	Q[iQueryNum].r = r;
	return itplIdx;
	}*/
	/*	TUPLE_POINTER *TuplePoint=new TUPLE_ID_ATTR_POINTER[];
	TUPLE_POINTER *r1,*r2;
	r1=TuplePoint;
	r1->d = -1;
	r1->next=NULL;
	...
	while (TRUE)
	{
		// iSizeofCandidateSet is Member Variable, number of bOK; iSizeofCandidateSet++ in ::zlGetTuplesInBall()
		...
		for (i = 0; i <COL_NUM; i++) //i means attri --- Attribute_i 
		{
			//the two intervals are [a2, a1) and (b1, b2] for each Attribute_i 
			ka[i]--; kb[i]++;
			...
			if (low >= 0 && upp >= 0)
			{
				if ((Qry.x[i] - L[i].t[low].a) >= (L[i].t[upp].a - Qry.x[i]))
				{
					location = upp;
					ka[i] ++;
					UpperBound[i] = L[i].t[upp].a - Qry.x[i];
				}
				else
				{
					location = low;
					kb[i] --;
					UpperBound[i] = Qry.x[i] - L[i].t[low].a;
				}
			...
			{
					  //---for Sum distance, L1
					  UpperBoundSum = UpperBoundSum + fabs(UpperBound[i]);
			...
			{
					  //----for Eucl distance, L2
					  UpperBoundSum = UpperBoundSum + UpperBound[i] * UpperBound[i];
			...
			{
					   //----for Max distance, Linf					
					   UpperBoundSum = (double)__max(UpperBoundSum, fabs(UpperBound[i]));
			...
			iSortedCount++;
			if (cID[L[i].t[location].ID] == 0) // -- if NOT in buffer, then insert into buffer 
			{
				cID[L[i].t[location].ID] = 1; //remove repeat tuples
				//if(L[i].t[ location ].ID==15982)
				//	AfxMessageBox("找到了");
				/* if(L[i].t[ location ].ID==2179)
				AfxMessageBox("找到了"); */

				//memset(&tt, 0, sizeof(TUPLE));

				iSeenCount++;

				tt.ID = L[i].t[location].ID;
				tt.x[i] = L[i].t[location].a; // Sorted Access


				for (k = 0; k<i; k++) // other Attribute index k , Random Access
				{
					tt.x[k] = tInD[tt.ID].x[k];
					iRandomCount++;
				}
				for (k = i + 1; k<COL_NUM; k++) // other Attribute index k, Random Access
				{
				...
				//tpl's index is based 0 		
			}
		}

		if (DIS_NUM == 2)
			UpperBoundSum = sqrt(UpperBoundSum);

		if (N <= UpperBoundSum && itplIdx >= iTopN)
		{
			//delete[] tt;
			//tt = NULL;		
			return  iTopN;  //zl. 2015.08.18
			//return 1;         //zl. 2015.08.18
		}
...
int CMainFrame::GetTASameResult_Square(QUERY Query, int iQueryNum, double dSd, int iSizeOf_n_square)  //this is the variant of  GetTASameResult
{
	int n = COL_NUM;
	QUERY Qry = Query;
	//ZLRECT S; //the search n-square of Qry;  S = [a(0), b(0)]x...x[a(n-1), b(n-1)]
	...
	iSizeofCandidateSet = 0;
	iSortedCount = iRandomCount = 0;
	/// iSizeOf_n_square = 0;  //-- 2015.08.18. zl
	int low, upp, location;
	double UpperBound[COL_NUM + 1];
	double UpperBoundSum;
	double N;
	...
	int *cIDtpl = new int[iSizeofDataSetTable + 1]; // be used to remove repeat tuples

	//iSizeOf_n_square = 0;  // in general , is NOT 0 	//-- 2015.08.16.

	//-- 2015.08.16. 1. we need to:   Location of  ( Qry.x[i] -r ) and (Qry.x[i] + r)  ---- start ----
	/// ==  ---------------------  for Qry.x[i] - dSd
	for (i = 0; i < COL_NUM; i++) //i means attri --- Attribute_i 
	{
		//Li.t[j] , j is based 0 , Li.t[j] from small to large  
		if (Qry.x[i] - dSd <= L[i].t[0].a)  // q[i] <= MIN(Ai), where  q[i] is Qry.x[i] 
		{
			ka[i] = -1;  //never access it,	
			a2[i] = a1[i] = Qry.x[i] - 10000 * dSd - 1000; // a2[i] = a1[i] = -o|o, infinite Large 
		}
		else if (Qry.x[i] - dSd >= L[i].t[iSizeofDataSetTable - 1].a) // q[i] >=  MAX(Ai)
		{
			ka[i] = iSizeofDataSetTable - 1; //will use it by ka[i]--
			a2[i] = a1[i] = L[i].t[iSizeofDataSetTable - 1].a;          // a2[i] = a1[i] = MAX(Ai)
		...
		{
			for (j = 0; j < iSizeofDataSetTable - 1; j++)   // since j+1 = iSizeofDataSetTable-1 //?? change to binary search 
			{
				//Li.t[j] , j is based 0; find out a[i][j]<=q[i] <=a[i][j+1]
				if ((L[i].t[j].a <= Qry.x[i] - dSd) && (Qry.x[i] - dSd <= L[i].t[j + 1].a))
				{
					ka[i] = j;      //save the location by ka[i], kb[i]
					a2[i] = a1[i] = L[i].t[j].a;  // save values that will be used to create two intervals 
			...
			}  //for (j = 0; j < iSizeofDataSetTable - 1; j++)
		}
	} // 
	/// == +++++++++++++++++++   Qry.x[i] + dSd
	for (i = 0; i < COL_NUM; i++) //i means attri --- Attribute_i 
	{
		...
		{
			kb[i] = iSizeofDataSetTable;  //will not access it  
			b2[i] = b1[i] = Qry.x[i] + 10000 * dSd + 1000;       // b2[i] = b1[i] = +o|o, infinite
		}
		else  // i.e., in the middle //  MIN(Ai) < q[i] < MAX(Ai)
		{
			for (j = 0; j < iSizeofDataSetTable - 1; j++)   // since j+1 = iSizeofDataSetTable-1 //?? change to binary search 
	...
	//-- 2015.08.16. we need to:   Location of  ( Qry.x[i] -r ) and (Qry.x[i] + r)  ---- end ----

	//2. assignments for 
	//pApp->tpl[i] is based 1,  but CMainFrame->tpl[i] is based 0

	for (ii = 0; ii<iSizeOf_n_square; ii++)
	{
		tpl[ii] = pApp->tpl[ii + 1];
		for (jj = 0; jj<COL_NUM; jj++)
			tpl[ii].x[jj] = pApp->tpl[ii + 1].x[jj + 1];

		tpl[ii].d = GetTupleD(tpl[ii], Qry);
		tpl[ii].d_max = tpl[ii].d;

		////////////////=======
		if (cID[tpl[ii].ID] == 0)
		{
			cID[tpl[ii].ID] = 1; //remove repeat tuples
			cIDtpl[tpl[ii].ID] = itplIdx;
...
int CMainFrame::zlGetCandidateSetBSR(QUERY Query, int iQueryNum)
{
	int n = COL_NUM;
	QUERY Qry = Query;
	ZLRECT S; //the search n-square of Qry;  S = [a(0), b(0)]x...x[a(n-1), b(n-1)]
	...
	tpl = new TUPLE[iSizeofDataSetTable + 1];   //candidate tuples for tpl[].bOK = true
	int itplIdx = 0;          // count for have been seen tuples, i.e., tpl[]	 
	for (i = 0; i<iSizeofDataSetTable; i++)
	{
		memset(&tpl[i], 0, sizeof(TUPLE));
	}
	...
	//char cID[581012]; // be used to remove repeat tuples
	//memset(cID, '\0', 581012);
	char *cID = new char[iSizeofDataSetTable + 1];
	memset(cID, '\0', iSizeofDataSetTable + 1);

	int *cIDtpl = new int[iSizeofDataSetTable + 1]; // be used to remove repeat tuples
	//memset(cIDtpl,0,iSizeofDataSetTable+1);

	iSizeofCandidateSet = 0;
	iSortedCount = iRandomCount = 0;

	iSeenCount = 0; //tuples number of have been seen
	...
	for (i = 0; i <COL_NUM; i++) //i means attri --- Attribute_i 
	{
		...
		else if (Qry.x[i] >= L[i].t[iSizeofDataSetTable - 1].a) // q[i] >=  MAX(Ai)
		{
			...
			//(1) Sorted Access, (2) Random Access
			// Random Access other attr _i , and save it in buffer: tpl[] 
			//if(L[i].t[ iSizeofDataSetTable-1].ID==1)
			//AfxMessageBox("zdl");
			if (cID[L[i].t[iSizeofDataSetTable - 1].ID] == 0)
			{
				cID[L[i].t[iSizeofDataSetTable - 1].ID] = 1; //remove repeat tuples
				...
				cIDtpl[L[i].t[iSizeofDataSetTable - 1].ID] = itplIdx;
		...
		else  // i.e., in the middle //  MIN(Ai) < q[i] < MAX(Ai)
		{
			for (j = 0; j < iSizeofDataSetTable - 1; j++)   // since j+1 = iSizeofDataSetTable-1
			{
				...
				if ((L[i].t[j].a <= Qry.x[i]) && (Qry.x[i] <= L[i].t[j + 1].a))
				{
					...
					//(1) Sorted Access, (2) Random Access 
					// for j 
					//	if(L[i].t[ j].ID==1 || L[i].t[ j+1].ID==1)
					//       AfxMessageBox("zdl");
					if (cID[L[i].t[j].ID] == 0)
					{
						cID[L[i].t[j].ID] = 1; //remove repeat tuples
	...
	r += r_maxmin; // r initialized 0
	for (i = 0; i <COL_NUM; i++) //i means attri --- Attribute_i 
	{
		S.a[i] = Qry.x[i] - r;
		S.b[i] = Qry.x[i] + r;
		//a2[i] = a1[i] - r;      // interval [a2, a1] 
		//a2[i] = a1[i] - (r- (Qry.x[i]-a1[i]));      // interval [a2, a1], at this time, a2[i] == Qry.x[i] - r;
		// b2[i] = b1[i] + r;     // interval [b1, b2]  ///
		//b2[i] = b1[i] + ( r - (b1[i] - Qry.x[i]) );    // interval [b1, b2]  /// b2[i] == Qry.x[i] + r;
		a2[i] = Qry.x[i] - r;
		a1[i] = __max(a1[i], a2[i]);
		b2[i] = Qry.x[i] + r;
		b1[i] = __min(b2[i], b1[i]);
	}
	...
	iSizeOfBall = zlGetTuplesInBall(tpl[0], itplIdx, Qry, iQueryNum, S, r);  // zlGetTuplesInBall  return iSizeofCandidateSet; 

	if (iSizeofCandidateSet >= iTopN)  //  iSizeOfBall == iSizeofCandidateSet; the number of tuples in ball , the ball is the subset of n-square
	{
		Q[iQueryNum].r = r;
		delete[] cIDtpl;
		delete[] cID;
	...
	int FristitplIdx;
	FristitplIdx = itplIdx;

	//find all tuples that = Q
	//if

	// 2. repeat to find Candidate Tuples

	bool bAllExhaust = false;
	...
	while (TRUE)
	{
		...
		if (iSizeofCandidateSet < iTopN)
		{
			...
			Q[iQueryNum].r = r;
			//// based on the existing n-square S,  for each [ai, bi] of S //// start 
			r_min = 0; r_maxmin = 0; // for next new n-square S,  for each [ai, bi] of S
			...
			for (i = 0; i <COL_NUM; i++) //i means attri --- Attribute_i 
			{
				...
				while (TRUE) ///////for ka[i]--; 
				{
					if (ka[i] >= 0)
					{
						...
						// ? L[i].t[ka[i]].a is between a2[i] and  a1[i]// i.e., ? in  [a2, a1)
						if (a2[i] <= L[i].t[ka[i]].a && L[i].t[ka[i]].a <= a1[i]) //if yes!
						{
							//For both sorted and random accesses
							//(1) Sorted Access, (2) Random Access 
							// about ka[i]
							...
							//if(L[i].t[ ka[i] ].ID == 13803)
							//AfxMessageBox("zdl");
							bInRect = true;
							if (cID[L[i].t[ka[i]].ID] == 0) // -- if NOT in buffer, then insert into buffer 
							{

								iSeenCount++;

								cID[L[i].t[ka[i]].ID] = 1; //remove repeat tuples
								tpl[itplIdx].ID = L[i].t[ka[i]].ID;
								tpl[itplIdx].x[i] = L[i].t[ka[i]].a; // Sorted Access
								tpl[itplIdx].d = -1;

								cIDtpl[L[i].t[ka[i]].ID] = itplIdx;
								...
								for (k = 0; k<i; k++) // other Attribute index k , Random Access
								{
									...
									if (tpl[itplIdx].x[k]< S.a[k] || S.b[k] < tpl[itplIdx].x[k])
									{
										bInRect = false;
										//break;   //for j
									}
								...
								{
									iSizeOf_n_square++;
									tpl[itplIdx].bIN = bInRect;
									tpl[itplIdx].d = zlComputeD(tpl[itplIdx], Qry);
									//itplIdx ++;
								}
								////// ? is it a top-N tuple ? 
								//if(! tpl[itplIdx].bOK) 
								//iSizeOfBall = zlGetTuplesInBall(tpl[itplIdx], itplIdx,  Qry, iQueryNum, S, r); 
								////// ? is it a top-N tuple ?
								itplIdx++;
								//tpl's index is based 0 		
							}
							...
							{
								if (tpl[cIDtpl[L[i].t[ka[i]].ID]].bIN == 0)
								{
									for (k = 0; k<COL_NUM; k++) // other Attribute index k, Random Access
									{
										//	tpl[itplIdx].x[k] = tInD[tpl[itplIdx].ID].x[k];
										//iRandomCount++;

										if (tpl[cIDtpl[L[i].t[ka[i]].ID]].x[k]< S.a[k] || S.b[k] < tpl[cIDtpl[L[i].t[ka[i]].ID]].x[k])
										{
											bInRect = false;
											break;   //for j
										}
									...
									{
										iSizeOf_n_square++;
										tpl[cIDtpl[L[i].t[ka[i]].ID]].bIN = bInRect;
										tpl[cIDtpl[L[i].t[ka[i]].ID]].d = zlComputeD(tpl[cIDtpl[L[i].t[ka[i]].ID]], Qry);
										//itplIdx ++;
									}
								}
							}

							ka[i]--;   /// sorted next value in Li
						}
						else if (L[i].t[ka[i]].a < a2[i]) // if NOT in  [a2, a1)
						{
							a1[i] = a2[i];  //closing a2[i] 
							a2[i] = L[i].t[ka[i]].a; // in order to computing r_min; NOT saved in tpl[ ];								 

							r_min = a1[i] - a2[i];
							// r_maxmin = __max(r_maxmin, r_min);			

							ka[i]++; // start from  this ka[i], IMportant

							break; //while (TRUE) //for ka[i]--; 
						}
				...
				} ////// match --- while (TRUE) //for ka[i]--; 


				while (TRUE) ///////for kb[i]++; 
				{
					if (kb[i] < iSizeofDataSetTable)  //// in the Domain 
					{
						// ? L[i].t[kb[i]].a is between b1[i] and  b2[i]// ?? in  (b1, b2]
						if (b1[i] <= L[i].t[kb[i]].a && L[i].t[kb[i]].a <= b2[i]) // IN (b1, b2]
						{
							//For both sorted and random accesses
							//(1) Sorted Access, (2) Random Access 
							// about ka[i] 
							iSortedCount++;
							//if(L[i].t[ kb[i] ].ID == 13803)
							// AfxMessageBox("zdl");

							bInRect = true;
							if (cID[L[i].t[kb[i]].ID] == 0) ///  if NOT in buffer, then insert into buffer 
							{

								iSeenCount++;

								cID[L[i].t[kb[i]].ID] = 1; //remove repeat tuples
								tpl[itplIdx].ID = L[i].t[kb[i]].ID;
								tpl[itplIdx].x[i] = L[i].t[kb[i]].a; // Sorted Access
								tpl[itplIdx].d = -1;

								cIDtpl[L[i].t[kb[i]].ID] = itplIdx;
								for (k = 0; k<i; k++) // other Attribute index k , Random Access
								{
									tpl[itplIdx].x[k] = tInD[tpl[itplIdx].ID].x[k];
									iRandomCount++;
							...
							{
								if (tpl[cIDtpl[L[i].t[kb[i]].ID]].bIN == 0)
								{
									for (k = 0; k<COL_NUM; k++) // other Attribute index k, Random Access
									{
										//	tpl[itplIdx].x[k] = tInD[tpl[itplIdx].ID].x[k];
										//iRandomCount++;
										// bInRect = true;
										if (tpl[cIDtpl[L[i].t[kb[i]].ID]].x[k]< S.a[k] || S.b[k] < tpl[cIDtpl[L[i].t[kb[i]].ID]].x[k])
										{
											bInRect = false;
											break;   //for j
										}
									...
									{
										iSizeOf_n_square++;
										tpl[cIDtpl[L[i].t[kb[i]].ID]].bIN = bInRect;
										tpl[cIDtpl[L[i].t[kb[i]].ID]].d = zlComputeD(tpl[cIDtpl[L[i].t[kb[i]].ID]], Qry);
										//itplIdx ++;
									}
								}
							}
						...
						else if (b2[i] < L[i].t[kb[i]].a)// NOT in (b1, b2], renew (b1, b2]
						{
							//sprintf(temp, "b1 = %f, b2= %f", b1[i],b2[i]);
							// AfxMessageBox(temp);
							b1[i] = b2[i];
							b2[i] = L[i].t[kb[i]].a;
							if ((r_min == 0) || (b2[i] - b1[i]) < r_min)
								r_min = b2[i] - b1[i];
							// r_maxmin = __max(r_maxmin, r_min);

							kb[i]--; // start from  this kb[i], IMportant

							break; ////while (TRUE) //for kb[i]++; 
						}

					}  //// -- match --- if( kb[i] < iSizeofDataSetTable)  //// in the Domain 
					else
					{
						kbExhaust[i] = true;
						break; ////while (TRUE) //for ka[i]++;
				...
				}  //// match ---while (TRUE) //for kb[i]++;
				//if(r_min != 0)
				r_maxmin = __max(r_maxmin, r_min);

			} // --- match ---for(i = 0; i <COL_NUM; i++) //i means attri --- Attribute_i 

			///////////must consider if all tuples have been seen,the number of square do not reach N



			////// 
			iSizeOfBall = zlGetTuplesInBall(tpl[itplIdx], FristitplIdx, Qry, iQueryNum, S, r);

			if (iSizeOf_n_square >= iTopN)
			{
				iSizeOfBall = zlGetTuplesInBall(tpl[itplIdx], itplIdx, Qry, iQueryNum, S, r); //bi xu you,ding wei shi ke neng quan bu shuxing yi bei fangwen.
				int p = DIS_NUM;
				if (p == -1)
				{
					delete[] cIDtpl;
			...
			for (j = 0; j<COL_NUM; j++)
			{
				if (!kaExhaust[j] || !kbExhaust[j])
					break;
			}
			if (j == COL_NUM)
				bAllExhaust = true;
			/*if(kaExhaust[0] && kbExhaust[0])
			bAllExhaust=true;*/

			if (iSizeofCandidateSet < iTopN)
			{
				if (bAllExhaust == true)
					//if(itplIdx == iSizeofDataSetTable)
				{
					while (iSizeofCandidateSet < iTopN)
					{
						//double EPCINO = 0.000000000000002; //15_0
						double EPCINO = 0.1; //15_0
						char cDblToStr[1024] = "";
						sprintf(cDblToStr, "%.0f", r);

						int ilen = strlen(cDblToStr);

						//r = r + EPCINO* pow(10.0, (ilen) );
						r = r + EPCINO* r;//hai xing
						// r=r*2;
						...
						if (iSizeOf_n_square >= iTopN && iSizeofCandidateSet < iTopN)
						{
							int p = DIS_NUM;
							r = r * pow(COL_NUM, 1.0 / p);
							for (i = 0; i < COL_NUM; i++) //i means attri --- Attribute_i 
							{
								S.a[i] = Qry.x[i] - r;
								S.b[i] = Qry.x[i] + r;
				...
				else  if (iSizeOf_n_square >= iTopN)
				{
					//while(iSizeofCandidateSet < iTopN)
					//	{
					int p = DIS_NUM;
					double pp = pow(COL_NUM, 1.0 / p);
					//double pp=1.1;
					r = r * pp;
					for (i = 0; i < COL_NUM; i++) //i means attri --- Attribute_i 
					{
			...
			//// based on the existing n-square S,  for each [ai, bi] of S //// end 

			//-------------------------------------------------------------------------------//
			//////////////////// construct the new  n-square S; i.e., enlarge n-square S
			///// get max from min ;  rj = max ( {min{qi - aij , aij+1 - qi}};  0 <= i < COL_NUM) , 
			//the two intervals are [a2, a1) and (b1, b2] for each Attribute_i 
			/*********
			for(i = 0; i <COL_NUM; i++) //i means attri --- Attribute_i
			{
			// we need to consider 3 cases , the 4th case will not probe ////////
			//// m == MIN(Ai); M == MAX(Ai)

			// case 1:  // a1[i] < MIN(Ai) and b1[i] <= MAX(Ai) / a1--m--b1--M
			//closed interval [MIN(Ai), b1[i]] has been searched

			if (a1[i] < L[i].t[0].a  &&  b1[i] <= L[i].t[iSizeofDataSetTable-1].a)
			{
			r_min  =  b2[i]- b1[i];
			r_maxmin = __max(r_maxmin, r_min);
			}

			// case 2: //  MIN(Ai)<= a1[i]  and  MAX(Ai) < b1[i] // m--a1--M--b1
			if ( L[i].t[0].a <= a1[i]  && L[i].t[iSizeofDataSetTable-1].a < b1[i])
			{

			r_min  =  a1[i]- a2[i];
			r_maxmin = __max(r_maxmin, r_min);
			}
			// case 3: // i.e., in the middle,  MIN(Ai)<= a1[i] and  b1[i] <= MAX(Ai) //m--a1--b1--M
			if( L[i].t[0].a <= a1[i] && b1[i] <= L[i].t[iSizeofDataSetTable-1].a )
			{
			r_min    = __min(a1[i]-a2[i], b2[i]-b1[i]);
			r_maxmin = __max(r_maxmin, r_min);
			}

			} // --- match -- second for(i = 0; i <COL_NUM; i++)
			************************************************************/

			if (r_maxmin <= 10e+23)
				r_maxmin = 10e+23;//仅限于高维！！！！！！！！！！！！！
			...
			for (i = 0; i < COL_NUM; i++) //i means attri --- Attribute_i 
			{
				S.a[i] = Qry.x[i] - r;
				S.b[i] = Qry.x[i] + r;
				a2[i] = Qry.x[i] - r;  // a1[i] -= r_maxmin;   // == S.a[i]
				b2[i] = Qry.x[i] + r;  // b1[i] += r_maxmin;   // == S.b[i] 
				a1[i] = __max(a1[i], a2[i]);
				b1[i] = __min(b2[i], b1[i]);
			}


		} ///--- match -- first --- if (iSizeofCandidateSet < iTopN)
		else
		{
			break; // while (TRUE)
		}

		// if( all of  kabExhaust[i] == true ) // if exhaust all   ka[i] < 0 ; kb[i]> = iSizeofDataSetTable;
		//for(i = 0; i < COL_NUM; i++) //i means attri --- Attribute_i 
		//{  
		//    bAllExhaust =  bAllExhaust && kaExhaust[i];
		//	bAllExhaust =  bAllExhaust && kbExhaust[i];
		//}
		//if(bAllExhaust)
		//       	break; // while (TRUE)
...
// zlGetCandidateSetBSR_Square is variant of zlGetCandidateSetBSR, that is , we have obtain an n_square S(Q,r) 
// and the tuples in it, but  S(Q,r) have not top-N tuples, then we  enlarge S(Q,r) such that    top_N <= iSizeOfBall == iSizeofCandidateSet; the number of tuples in ball , the ball is the subset of n-square
// double dSd: dSd means search distance. 
// this fnction is important, and major jobs.
// whether or not we need to use the function : tpl[itplIdx].d = zlComputeD(tpl[itplIdx], Qry);
//Get candidate set of the query Q[i] by Both Sorted and Random, BSR means Both Sorted and Random 
int CMainFrame::zlGetCandidateSetBSR_Square(QUERY Query, int iQueryNum, double dSd, int iSizeOf_n_square)
{
	int n = COL_NUM;
	QUERY Qry = Query;
	ZLRECT S; //the search n-square of Qry;  S = [a(0), b(0)]x...x[a(n-1), b(n-1)]
...
int CMainFrame::zlGetTuplesInBall(TUPLE tplseen, int tupleNum, QUERY Query, int iQueryNum, ZLRECT S, double r)
{
	bool bInRect;
	int i, j;
	double x[COL_NUM + 1];
	QUERY Qry = Query;
	iSizeofCandidateSet = 0; //2016.0809 by song
	/// here 2015.08.15

	for (i = 0; i <tupleNum; i++)/// here may have problem!!!!!!!!!!!!!!!
	{
		//if(i==8681)
		//	{
		//  bInRect = true;
		...
		if (!tpl[i].bIN)
		{
			...
			for (j = 0; j < COL_NUM; j++)
			{
				if (tpl[i].x[j]< S.a[j] || S.b[j] < tpl[i].x[j])
				{
					bInRect = false;
					break;   //for j
				}
			...
			if (bInRect)
			{
				tpl[i].bIN = bInRect; // tpl[i] is in n-square S   
				iSizeOf_n_square++;
				//  if(iSizeOf_n_square==21)
				//	AfxMessageBox("!!!");

				for (j = 0; j<COL_NUM; j++)
					x[j] = fabs(tpl[i].x[j] - Qry.x[j]);
				...
				case -1:
				{
						   ...
						   tpl[i].d = x[0];
						   for (j = 1; j< COL_NUM; j++)
							   tpl[i].d = (double)__max(tpl[i].d, x[j]);
...
int CMainFrame::zlGetTopNTupleUniA(int QueryNum, int tuplesNum, QUERY Query)
{
	int iCandidateNum = -1;
	int iFrequence_Opt = -1;
	...
	//1. Obtain all distance From Query to tuples
	//#define  DIS_NUM   2 //-1 //2 ////: 1 == L1; 2 == L2; -1 =Linf

	int iRowCnt = tuplesNum;  // the size of buffer, i.e., the number of tpl[].
	...
	/*	delete [] t;  /// top-N tuples
	t = new TUPLE[iSizeofCandidateSet]; */
	...
	//double r[iCandidateNum+1];     //
	//double dis[iCandidateNum+1];
	...
	iCandidateNum = j; //it should be iSizeofCandidateSet

	if (iCandidateNum < iTopN)
		tupCount = iTopN;
	else
		tupCount = iCandidateNum;

	double * r = new double[tupCount + 1];
	double * dis = new double[tupCount + 1];
	int * tIndex = new int[tupCount + 1]; //or 2000


	for (j = 0; j<tupCount; j++)
	{
		dis[j] = t[j].d;
	}
	...
	*/
	...
	for (j = 0; j<iTopN; j++)
	{
		for (k = j + 1; k<iCandidateNum; k++)
		{
			if (t[k].d < t[j].d)
			{
				//ftmp     = r[j];     r[j]   = dis[k];    dis[k] = ftmp;
				TmpTuple = t[j];     t[j] = t[k];      t[k] = TmpTuple;
				iNumTmp = t[j].num; t[j].num = t[k].num;  t[k].num = iNumTmp;
				//tIndex[j]=k;		
			}
		}
	...
	/*	if(iCandidateNum < iTopN)
	{
	for(i =0; i <iRowCnt; i++)/// here may have problem!!!!!!!!!!!!!!!
	{
	if(! tpl[i].bIN)
	{
	for(j = 0; j<COL_NUM; j++)
	x[j] = fabs( tpl[i].x[j] - Query.x[j] );
...
double CMainFrame::GetTupleD(TUPLE tplseen, QUERY Query)
{
	int j;
	double x[COL_NUM + 1];
	QUERY Qry = Query;
...
double CMainFrame::zlComputeD(TUPLE tplseen, QUERY Query)
{
	//bool bInRect; 
	int j;
	double x[COL_NUM + 1];
	...
	case -1:
	{
			   ...
			   tplseen.d = x[0];
			   for (j = 1; j< COL_NUM; j++)
				   tplseen.d = (double)__max(tplseen.d, x[j]);
...
int CMainFrame::zlInitFunction()
{
	...
	//*  1. select all tuples from one of 8 base tables (Attr_Census2D, …, Lsi_104D)
	//-----   Call function: int zlSelectAllFromDataSetTable(char * DataSetTable )
	// We get all tuples in DataSet:	tInD[i], i= 1, 2, ..., |D|
	iCStrLenth = m_DataSetTable.GetLength() + 1;
	iSizeofDataSetTable = zlSelectAllFromDataSetTable(m_DataSetTable.GetBuffer(iCStrLenth));


	if (iSizeofDataSetTable <= 0)
	{
		AfxMessageBox(" Call zlSelectAllFromDataSetTable Error !!!!!!");
	}
	else
	{
		sprintf(temp, " select all tuples from m_DataSetTable, iSizeofDataSetTable = %i", iSizeofDataSetTable);
		AfxMessageBox(temp);
	}

	//*  2. create Li (i =1 to COL_NUM) by using SQL select statements from one of 8 base tables
	//-----   Call function: int zlSelectOneAttrFromDataSetTable(char * DataSetTable )
	// We get all queries in the workload:	Q[i], i=0,1, 2, ..., 9999

	for (i = 0; i <COL_NUM; i++)
	{
		iSizeofList = zlSelectOneAttrFromTable(m_DataSetTable.GetBuffer(iCStrLenth), i); //(table name, attr#)

		if (iSizeofList != iSizeofDataSetTable)
		{
			sprintf(temp, " Call zlSelectOneAttrFromTable Error !!! iSizeofList = %i", iSizeofDataSetTable);
			AfxMessageBox(temp);
		}
		else

		{
			//sprintf(temp, " 2. create Li, iSizeofList = %i", iSizeofList);
			//AfxMessageBox(temp);
		}
	...
	/*************************
	iCStrLenth = m_WorkloadTable.GetLength() + 1;
	int iSizeofWorkload = zlSelectAllFromWorkload(m_WorkloadTable.GetBuffer(iCStrLenth));
	...
	{
	sprintf(temp, " 3. Select all queires from workload table, iSizeofWorkload = %i", iSizeofWorkload);
	AfxMessageBox(temp);
	}
	****************************/
...
int CMainFrame::zlSelectAllFromDataSetTable(char * DataSetTable)
{
	int iRowCnt = -1;
	int iTupleNum;
	...
	char szLocalSql[2048] = "";
	strcpy(szLocalSql, "SELECT * FROM ");
	strcat(szLocalSql, DataSetTable);
	...
	{  //	retcode = SQLExecDirect
		...
		delete[] tInD;
		tInD = new TUPLE[iRowCount + 1];
		memset(tInD, 0, sizeof(TUPLE)*(iRowCount + 1));


		for (i = 0; i <COL_NUM; i++)
		{
			L[i].t = new TUPLE_ID_ATTR[iRowCount + 1];
		}
		...
		while (TRUE)
		{
			...
			if (retcode == SQL_SUCCESS || retcode == SQL_SUCCESS_WITH_INFO)
			{
				...
				else
				{
					for (i = 0; i<COL_NUM; i++)
					{
						tInD[sColID].x[i] = fCol[i];
					}

					tInD[sColID].ID = sColID;
	...
	for (i = 1; i <= iRowCnt; i++)
	{
		if (tInD[i].ID != i)
		{
			wsprintf(tmp, " tInD[i].%ld <> %ld", tInD[i].ID, i);
			AfxMessageBox(tmp);
...
// zlSelectOneAttrFromTable
//input : iAttrNum= 0; iAttrNum <COL_NUM; 
int CMainFrame::zlSelectOneAttrFromTable(char *DataSetTable, int iAttrNum)
{
	int iRowCnt = -1;
	int iTupleNum;
	...
	//SELECT * FROM attr_census3d  ORDER BY attr0 DESC  //Not use   DESC
	//SELECT * FROM attr_census3d  ORDER BY attr0       // We use this

	char szLocalSql[2048] = "";
	strcpy(szLocalSql, "SELECT * FROM ");
	strcat(szLocalSql, DataSetTable);
	wsprintf(tmp, " ORDER BY attr%ld ", iAttrNum);
	strcat(szLocalSql, tmp);
	...
	{  //	retcode = SQLExecDirect
		...
		if (iRowCnt <= 0 || iRowCnt != iSizeofDataSetTable)
		{
			AfxMessageBox("iRowCnt != iSizeofDataSetTable, Error !!!!!!");
			return iRowCnt;
		}
		...
		while (TRUE)
		{
			...
			if (retcode == SQL_SUCCESS || retcode == SQL_SUCCESS_WITH_INFO)
			{
				...
				else
				{
					...
					L[iAttrNum].t[iQryIdx].a = fCol[iAttrNum];

					L[iAttrNum].t[iQryIdx].ID = sColID;
	...
	/////// test 
	for (i = 0; i<iRowCnt - 1; i++)
	{
		if (L[iAttrNum].t[i].a > L[iAttrNum].t[i + 1].a)
		{
			sprintf(tmp, " t[%ld].a = %G; t[%ld].a = %G", i, L[iAttrNum].t[i].a, i + 1, L[iAttrNum].t[i + 1].a);
			AfxMessageBox(tmp);
	...
	pApp->dMin[iAttrNum + 1] = L[iAttrNum].t[0].a;
	pApp->dMax[iAttrNum + 1] = L[iAttrNum].t[iRowCnt - 1].a;
