...
////////////// zl veriable defination start //////////////////////
	...
	TUPLE * tInD;       //all tuples from DataSet 2015.7.29 songxin
	// tInD[sColID],  sColID = 1, ..., |dataset|
	// tInD[sColID].x[i] = fCol[i]; i = 0, ... , i<COL_NUM ; 
...
BOOL CLearnIing6App::InitInstance()
{
	...
	//strcpy((char *)sz_DSN , "DSN_QueryStream");  //DSN_MULTopN, DSN_Census2D
	strcpy((char *)sz_DSN , "DSN_QueryStream128");  // 2015.04. 02 ; to use "DSN_KNNStream"
	                                           // since size of "DSN_TopNQueryStream20130202" > 20 
	strcpy((char *)sz_UID , "sa");
	strcpy((char *)sz_PWD , "sxzhy+dd"); //txgstxh
...
strcpy(szSql, "USE master \
 ...
 RECONFIGURE WITH OVERRIDE \
 EXEC sp_configure 'max server memory', '8' \
 RECONFIGURE WITH OVERRIDE USE MulTopN ");
 //--- Note: here database is MulTopN, not QueryStream
...
int CLearnIing6App::zlPartion3D1(ZLRECT Rect, int Dimention, ZLRECT * T, int T_Num, int PartNum)
{
		//CLibServerApp * pApp = (CLibServerApp * ) AfxGetApp();
	//bReturn = pApp->zlExeUpdateTable((char *)szSqlUpdate);			
...
////// ~~~~~~~~ Finding all T[j]'s that center are in part[i].
	for(i= 1; i<=12; i++)
	{
		...
		if(idx>=8)
		{
			...
			// call this function again.
           PartNum = zlPartion3D1(part[i], n, T, T_Num, PartNum);
...
int CLearnIing6App::zlGetCandidateSet(QUERY Query, double r)
{
	...
	int iMaxTupleNum = 0; // zl 2015.08.11, used for the next step, if  "IJCIS paper Fig. 7a. Increasing r to guarantee |S(Q, r)| >= 10 with a restart" fails, 
	// find another SCR, such that distance(SCR.center - S.center) is the smallest among all of them.
	int iInterIdx = 0;
	bool bInterable = false;
	int iBSRIdx = 0;  //BSR is Best Super Region, this is the bsn.
	int iWSRIdx = 0; //WSR is the worst Super Region that contain S, that is WSR has the most number of tuples in all superset of S.
	// iWSRIdx is the index number of WSR. // zl 2015.08.11
	bool bBSRable = false;
	...
	//From Ξ, find out all regions that intersect with S(Q, r),  
	//denote as {SCR k  | SCR k  S(Q, r)   , k = 1,…, K }
	//-------------------------------------------

	//1. Finding {scr_CLS[k]} that is super sets of S 
	int iInsctNum = 0;
	...
	for (j = 1; j <= clsNum; j++)   // when i=5, j = 1,2,3,4 if iStart =1
	{
		if (scr_CLS[j].tplNum > 0)
		{
			...
			//  then  call the algorithm GTA directly and other functions of Liu Feifei to get candidate and top-N tuples. 
			//?????????????????????????
			//GTA   //2015.08.11 
			ituples_cnt = 0;
			//?????????????????????????
	...
	//从一组包含查询区域的聚类区域SCR中，找到最佳的，叫做BSR
	if (bS_in_scr_CLS)
	{//-- Find out the BSR, where BSR means best super region
		for (j = 1; j <= clsNum; j++)
		{
			if (SuperSetIdx[j] >0)
			{
	...
	// If we get the SCR of S, then 
	//-- get candidate tuples:  pApp->tpl[i]  ---
	if (bS_in_scr_CLS) //S is the subset of "the smallest SCR": scr_CLS[iInterIdx]
	{
		S.bsn = iBSRIdx; //表示查询Q对应的查询区域S的最佳聚类区域SCR是iBSRIdx
		scr_CLS[0] = S;
