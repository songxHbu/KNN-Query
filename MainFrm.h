...
public:
	...
	int iSizeofDataSetTable; // the size of DataSet/Table songxin
	LIST * L;           // List that comtains tupleID and one of attributes. songxin
...
public:
	...
	//---- 2015.08.12 start 
	int iSizeofCandidateSet;
	int iSizeOf_n_square;
	int  iSizeOfBall;  //the number of tuples in the ball, which is circle or diamond. 
	int iAnswerNum;  // the final size of top-N tuples.
	int iQueryNum;
	BOOL iNoEffect;

	int iSortedCount, iRandomCount;  // count of sorted and/or random accesses

	int iSeenCount;

	bool bMTA;  // bMTA == true, run mTA-algorithm
	bool bGTA;  // bGTA == true, run GTA-algorithm
...
// Implementation
public:
	int zlGetTopNTupleOfProfileQuery(int QueryNum, int tuplesNum); //2015.07.30. zl add. get top-N tuples for each Profile query.
...
protected:
	...
	DECLARE_MESSAGE_MAP()
public:
	int zlGetCandidateSetBSR(QUERY Query, int iQueryNum); //  Get candidate set of the query Q[i] by Both Sorted and Random (BSR)
	int zlGetCandidateSetBSR_Square(QUERY Query, int iQueryNum, double dSd, int iSizeOf_n_square); //this is the variant of  zlGetCandidateSetBSR
	int GetTASameResult(QUERY Query, int iQueryNum); //this is the mTA algorithm, comes from Liu Feifei.
	int GetTASameResult_Square(QUERY Query, int iQueryNum, double dSd, int iSizeOf_n_square);   //this is the variant of  GetTASameResult
	int zlGetTuplesInBall(TUPLE tplseen, int tupleNum, QUERY Query, int iQueryNum, ZLRECT S, double r);
	int zlGetTopNTupleUniA(int QueryNum, int tuplesNum, QUERY Query);
	double GetTupleD(TUPLE tplseen, QUERY Query);
	double zlComputeD(TUPLE tplseen, QUERY Query);
	int zlInitFunction();
	int zlSelectAllFromDataSetTable(char * DataSetTable);
	int zlSelectOneAttrFromTable(char * DataSetTable, int iAttrNum);
	afx_msg void OnKnnGTA();
};
